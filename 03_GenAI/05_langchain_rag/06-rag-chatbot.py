# LLM이 사용자의 질문에 대한 답변을 바로 하지 않고  PDF문서에서 검색하여 대답할 수 있는 RAG[검색증강생성]챗봇 만들기 -- 채팅 UI는 Gradio 모듈 이용.
# RAG 구현 워크플로(파이프라인) 설계
# 사용자 입력 --> 프롬프트 탬플릿 --> vector DB에서 검색 (retrieval)--> LLM에게 답변 요청 --> 사용자 

#본인이 한것을 말해봐라라고 면접관이 이야기 했다고 함.
...
#3. 저장된 벡터 DB가 있다면 불러와서 데이터를 검색해오는 검색기(리트리버)로 만들기 -이거는 알려준적 없다. (vector_db.as_retriever 로 하면 알아서 함. ) vector_db.as_retriever 이 결국 강아지이고 vector_db를 어디거에 따라 달라지는 것이고 뭐를 쓰든 as retirever가 됨.

#WORK #2. [구성요소2]두뇌(모델)
#4. 질문에 답변을 생성하는 LLM model 생성 (랭체인 ChatOpenAI)

#WORK #3. [구성요소3] 지침 및 입력(사용자 질문을 모델이 잘 수행하다록..)
#5. 프롬프트 탬플릿으로 프롬프트를 정의 - 사용자 입력을 구조화



#WORK #4. 위 3개의 구성요소(Runnable) [1]+[2]+[3]을 체인으로 연결 ~ 책에 있는데로 하면 안되고 새로 라이브러리 설치 pip install lanchain-classic
#6. 모델과 지침은 먼저 체인으로 연결 [2] + [3] ~ LCEL [ model | prompt] ~ 이것도 권장 안한다 해서 최신것으로 : create_stuff_documents_chain()으로 바뀜 , AI가 발전하면서 자꾸 바뀜
...
#WORK #6. [RAG AI 챗봇 UI 제작] pip install gradio
#streamlit 대신 gradio 모듈로 UI 만들기
#-------------------------------------------------------------------------------------------
...
print('문서의 개수:',len(docs)) #페이지로 되어 있음.  #PDF 문서에서 필요없는 페이지를 제거하는 정제작업 필요.. 우리는 시간이 없지만 실제로 제거해야 한다. 필요없는 페이지를 ...예를 들면 맨뒤 발행년도등은 문서내용과 상관이 없어 제거해야 함.
...
import os
db_path='./05_langchain_rag/chroma_vector_db_financial.db'  # chroma_vector_db_financial.db 는 폴더임. 
...
retriever = vector_db.as_retriever(search_kwargs={'k':3}) #유사도상위 3개문서를 검색 kwargs: knowledge argument # 넌 이제 리트리버야.. 라고 해서 리트리버를 만듦. 
#---------------------------------------------------
